...
def get_total_pending(conn):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) FROM cities WHERE translations IS NULL OR translations = ''")
...
def main():
    ...
    print(f"Found {total} cities to translate in this batch\n")
    counter = 0
    ...
    for city in cities:
        ...
        # Cache to avoid duplicate requests for normalized language codes for THIS city only
        normalized_cache = {}
        ...
        for lang_code in languages:
            ...
            if normalized_lang_code in normalized_cache:
                ...
                if not use_transliteration:
                    translated = clean_translated_name(translated, city_name)
        ...
        cursor.execute("UPDATE cities SET translations = %s WHERE id = %s", (translations_json, city_id))

        # Commit after each city to avoid losing progress
        conn.commit()
        ...
        if counter % 10 == 0:
            ...
            avg_time_per_city = elapsed / counter
            remaining_cities = total - counter
            eta_seconds = avg_time_per_city * remaining_cities
            eta = str(datetime.timedelta(seconds=int(eta_seconds)))
            print(f"\n  ⏱️  Average: {avg_time_per_city:.1f}s per city | ETA: {eta}")
